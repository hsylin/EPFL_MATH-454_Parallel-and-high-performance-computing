...
void
XDMFWriter::write_mask_data(const std::vector<std::uint8_t> & mask)
{
  if (mask.size() != nx_ * ny_) {
    std::cerr << "Invalid solid mask size for XDMFWriter::write_mask_data\n";
    return;
  }
...
{
  // Serial/single-rank helper only. In multi-rank hybrid runs, rank 0 cannot
  // read other ranks' local rows through solver.rho()/ux()/uy()/vorticity().
  // Use LBM::gather_fields() followed by write_snapshot_data() instead.
...
void
XDMFWriter::write_snapshot_data(const std::vector<double> & rho_v,
                                ...
                                double t)
{
  ...
  if (rho_v.size() != expected || ux_v.size() != expected ||
      uy_v.size() != expected || vor_v.size() != expected) {
    std::cerr << "Invalid field size for XDMFWriter::write_snapshot_data\n";
    return;
  }
