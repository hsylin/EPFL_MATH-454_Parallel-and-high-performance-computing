...
#
# Dependency-free. Handles the exact Markdown generated by make_table.py:
# H1/H2 headings, blockquote summary, pipe tables, inline code, bold, notes.
#
# Usage:
...
def inline_md(s: str) -> str:
    # Convert only what is needed: code spans and bold.
    # Important: code spans are protected first so underscores inside config
    # names are not converted to <em>.
    s = s.rstrip()
    s = re.sub(r" {2,}$", "", s)
...
def is_separator_row(line: str) -> bool:
    # Accept cells such as -----:, :-------, --:, :-----
    # make_table.py uses '--:' for the p column, so do not require 3 dashes.
...
<head>
  ...
  <title>LBM MPI Scaling Results</title>
  <style>
    :root {{
      --bg: #f6f8fb;
      --card: #ffffff;
...
<main>
{body}
  <footer>Generated from LBM MPI scaling logs.</footer>
</main>
</body>
</html>
'''
