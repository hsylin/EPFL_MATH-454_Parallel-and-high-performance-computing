...
def main():
    ...
    t  = step[n0:].astype(float)
    s  = uy[n0:] - np.mean(uy[n0:])     # remove DC
    n  = len(s)

    # The CSV stores the true simulation step of each probe sample. Therefore
    # the FFT sample spacing must be inferred from the step column. This keeps
    # the Strouhal estimate correct when the solver uses probe_every > 1.
    dstep = np.diff(t)
    if len(dstep) == 0 or np.any(dstep <= 0):
        sys.exit("Probe step column must be strictly increasing.")
    dt = float(np.mean(dstep))
    if not np.allclose(dstep, dt, rtol=1e-6, atol=1e-12):
        sys.exit("Probe samples are not uniformly spaced; FFT requires a uniform sampling interval.")
    ...
    print(f"sample dt    : {dt:g}  lattice steps")
